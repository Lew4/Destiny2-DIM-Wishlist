...
工作簿左侧 B–I/PVE 和 K–R/PVP 图例中的说明文字会按图标关联到具体
perk 组合，并写入每条 DIM 规则前的 `//notes:`；没有图例说明的 perk 仍会
保留 `tags:pve` 或 `tags:pvp`。
...
测试覆盖文字表解析、signed/unsigned hash、旧版本兼容降级、真实样例XLSX的drawing提取、图标平移容忍、错列自动归位、同名武器类型纠正和特殊二号位普通/强化perk选择。当前完整真实Manifest回归基线为：4120个drawing、3221个perk图标位置、184/184个唯一perk图标识别成功；6326条组合审计记录经DIM语义去重并合并备注后输出6210条Wishlist规则。
