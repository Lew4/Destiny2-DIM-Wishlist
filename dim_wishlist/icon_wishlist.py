...
from .icon_reports import select_weapon_versions
...
def build_matches_and_wishlist(
    ...
    for (_, excel_row, weapon_name, usage), group in group_contexts(contexts).items():
        weapon_candidates = index.find_weapons(weapon_name)
        weapons = select_weapon_versions(config, weapon_name, weapon_candidates)
        if not weapons:
            for context in group:
                unresolved.append({
                    "excel_row": excel_row,
        ...
        for weapon in weapons:
            ...
            for context in group:
                ...
                row = {
                    ...
                    "accepted": "yes" if selected is not None else "no",
                    "reason": reason,
                    "recognized_names": " / ".join(visual.names) if visual else "",
            ...
            wishlist_lines.append(
                f"// {sanitize_comment(weapon_name)} [{weapon.hash}] ({usage}){suffix}"
            )
